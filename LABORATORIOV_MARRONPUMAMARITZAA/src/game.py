import random

words = ["meow", "cats", "claudita", "aea"]
word = random.choice(words)
guessed_letters = []
attempts = 5

def hide_letters(word):
    num_letters_to_hide = max(1, int(len(word) * 0.6))
    hidden_word = list(word)
    indices_to_hide = random.sample(range(len(word)), num_letters_to_hide)
    for index in indices_to_hide:
        hidden_word[index] = '_'
    return ''.join(hidden_word)

hidden_word_start = hide_letters(word)
print("Palabra inicial:", hidden_word_start)

while attempts > 0:
    hidden_letter = ''.join([c if c in guessed_letters else '_' for c in word])
    print("Palabra:", hidden_letter)
    print("Intentos restantes:", attempts)

    letter = input("Ingrese una letra o la palabra completa: ").lower()

    if len(letter) == 1 and letter.isalpha():
        guessed_letters.append(letter)
        if letter not in word:
            attempts -= 1
    elif len(letter) == len(word):
        if letter == word:
            print("¡Ganaste! Adivinaste la palabra:", word)
            break
        else:
            attempts -= 1
            print("Intento fallido. La palabra no es correcta.")
    else:
        print("Entrada inválida. Debes ingresar una letra o intentar adivinar la palabra completa.")

    if '_' not in hidden_letter:
        print("¡Felicidades! Adivinaste la palabra:", word)
        break

if attempts == 0:
    print("Perdiste. La palabra era:", word)
